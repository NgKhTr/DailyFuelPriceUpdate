...
update_headers = {
    "Authorization": f"Bearer {constants.NOTION_UPDATE_API_KEY}",
    "Notion-Version": "2022-06-28",
    "Content-Type": "application/json",
}
log_headers = {
    "Authorization": f"Bearer {constants.NOTION_LOG_API_KEY}",
    "Notion-Version": "2022-06-28",
    "Content-Type": "application/json",
}
...
def log(time: str, content: str):
    ...
    response = requests.patch(url, headers=log_headers, data=json.dumps(data))
...
class Board:
    ...
    def update_gia_hien_tai(gia_hien_tai: float, block_id: str):
        ...
        response = requests.patch(url, headers=update_headers, data=json.dumps(data))
        print(response)
...
def get_user_ids():
    ...
    response = requests.get(url, headers=update_headers) # 2 cái API xài cái nào cũng được
    data = response.json()
    user_ids = [
...
def clear_mentions():
    ...
    response = requests.get(url, headers=update_headers)
    data = response.json()
    ...
    for mention_block_id in mention_block_ids:
        ...
        response = requests.delete(url, headers=update_headers)
        print(response)


def send_mentions(user_ids, headers):
    data = {
        "children": [
            {
                "object": "block",
...
def send_update_notification(): # Trước đó là send_notification()
    # Lấy các user id
    user_ids = get_user_ids()
    # Xoá các child
    clear_mentions()
    # Thêm lại các child
    send_mentions(user_ids, update_headers)
...
def send_log_notification():
    ...
    clear_mentions()
    # Thêm lại các child
    send_mentions(user_ids, log_headers)
