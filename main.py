...
def task():
    ...
    if not is_url_accessible:
        ...
        if old_data is None or len(old_data) == 0:
            ...
            Notion_helper.send_update_notification()
        else:
            latest_date = pd.to_datetime(old_data[-1]["Date"]) # Này nó sẽ là đầu ngày, còn trong kia thì là giữa ngày, để chuẩn thì cái crawl về sẽ chuyển tất cả thành đầu ngày
